<?php
require_once "../dbcontroller.php"; // pastikan file koneksi sudah benar

$db = new DBController();
$error = "";

// Ambil data kategori dan brand/type untuk select
$kategorirow = $db->executeSelectQuery("SELECT id, kategori FROM kategori ORDER BY kategori ASC");
$brandrow = $db->executeSelectQuery("SELECT id, nama_type FROM type ORDER BY nama_type ASC");
...
if (isset($_POST['simpan'])) {
    ...
    $link = trim($_POST['detail']); // jika mau link terpisah dari description, ubah name di HTML
    $description = trim($_POST['detail']); // sesuai HTML, description dan link pakai 'detail', perlu beda name agar tidak tertimpa

    // Validasi
    if (empty($kategori) || empty($type) || empty($nama) || empty($link) || empty($description)) {
    ...
    if (empty($error)) {
        $sql = "INSERT INTO product (id_kategori, id_type, nama_produk, link, description, gambar, created_at, updated_at) 
                VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())";

        $params = [$kategori, $type, $nama, $link, $description, $gambar];
        $insert = $db->executeQuery($sql, $params); // executeQuery pake prepared statement
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <textarea id="detail" name="detail" required placeholder="Enter product link" class="form-control" rows="3"></textarea>
                                            </div>

                                            <div class="mb-3">
                                                <label for="detail" class="form-label">Product Description *</label>
                                                <textarea id="detail" name="detail" required placeholder="Enter product description" class="form-control" rows="3"></textarea>
                                            </div>
                                        </div>
